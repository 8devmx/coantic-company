...
$(document).on('change', 'input:checkbox[name=visibilidad-pro]', function (e) {
  ...
  $.post('modelo.php', obj, function (data) {
    //console.warn("modelo...response: "+data);
    if (data == 1) {
      listarDatos();
...
$(".cerrar").click(function () {
  ...
  tinyMCE.get('beneficios').setContent(cadena_bullets);
  tinyMCE.get('acerca').setContent(cadena_bullets);
  tinyMCE.get('caracteristicas').setContent(cadena_bullets);
  tinyMCE.get('extras').setContent(cadena_bullets);
  $("#downloadsLists").html("")
});
...
$('.guardar-datos, .editar-datos').on('click', function () { // guardar-datos: REGISTRAR, editar-datos: GUARDAR
  ...
  const textareas = ['descripcion', 'beneficios', 'acerca', 'caracteristicas', 'extras']
...
$(document).on('click', '.editar-elemento', function () {
  ...
  if (id > 0) {
    ...
    tinyMCE.get('acerca').setContent('');
    tinyMCE.get('caracteristicas').setContent('');
    tinyMCE.get('extras').setContent('');
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data[0].descripcion !== "" && data[0].descripcion !== null) {
        ...
        if (data[0].acerca !== "" && data[0].acerca !== null) {
          tinyMCE.get('acerca').setContent(data[0].acerca);
        }

        if (data[0].caracteristicas !== "" && data[0].caracteristicas !== null) {
          tinyMCE.get('caracteristicas').setContent(data[0].caracteristicas);
        }
        if (data[0].extras !== "" && data[0].extras !== null) {
          tinyMCE.get('extras').setContent(data[0].extras);
        }

        $("#respuestafoto1").css({ "z-index": 3 });
        $("#foto1").val(data[0].hero);
        $("#respuestafoto1").html(`
          <a title='Eliminar imagen' class='eliminarImagen' data-inp='foto1' data-img='1'>
            <i class='fa fa-remove' aria-hidden='true'></i>
          </a>
          <a class='linkimg' href='${data[0].hero_url}' target='_blank'>
            <img src='${data[0].hero_url}' class='foto-foto'>
          </a>`);
        $("#respuestapdf").css({ "z-index": 3 });
        $("#nombrepdf").val(data[0].hero);
        $("#respuestapdf").html(`
          <a title='Eliminar imagen' class='eliminarImagen' data-inp='nombrepdf' data-img='2'>
            <i class='fa fa-remove' aria-hidden='true'></i>
          </a>
          <a class='linkimg' href='${data[0].beneficios_imagen_url}' target='_blank'>
            <img src='${data[0].beneficios_imagen_url}' class='foto-foto'>
          </a>`);
              ...
              if (fotos[i] !== "") {
                $('#gale').append(`
                  <li id="imagen-${data[0].id}" class="ui-sortable-handle">
                    <a href="../../../uploads/servicios/thumb/${fotos[i]}" class="galeria">
                      <span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
                      <div class="view view-first">
                        <img src="../../../uploads/servicios/thumb/${fotos[i]}" alt="imagenes">
                        <div class="mask">
    ...
    success: function (response) {
      ...
      if ($.isEmptyObject(response)) {
        ...
        $.each(response, function (i, item) {
          ...
          html += '   <td style="width: 55%;"><a href="' + $urlsitio + 'servicios/' + item.url + '" target="_blank">' + item.titulo + '</a></td>';
