...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $nuevoNombre = $nombre_tem . "." . $extension;
    $directorio = "../../../uploads/vacantes/";
    $ruta_archivo = "../../../uploads/vacantes/" . $nuevoNombre;

    if ($extension == "png" || $extension == "jpeg" || $extension == "jpg" || $extension == "gif" || $extension == "svg") {

        if (!is_dir($directorio)) {
            mkdir($directorio, 0777);
            mkdir($directorio . "thumb/", 0777);
        }

        if ($nombre && move_uploaded_file($nombre_tmp, $ruta_archivo)) {

            create_square_image($ruta_archivo, $directorio . "thumb/" . $nuevoNombre, 720); //150x150 REVISAR FUNCION YA QUE MARCA ERROR: 1 NOV 2018
        ...
        $array["url_imagen"] = "";
        $array["extension"] = $extension;
        $array["error"] = "No es un archivo valido (" . $extension . ")";
    }
