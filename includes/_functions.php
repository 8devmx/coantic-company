...
function contacto()
{
  ...
  $de = $datos['Email'];
  $subjet = "Prospecto de Coantic Company ";

  $para = $_SESSION["correosPara"];
  $copiaCC = $_SESSION["correosCopia"];
...
function envio_email($para, $copiaCC, $copiaCCO, $de, $msg)
{
  ...
  if ($_SERVER['HTTP_HOST'] == "localhost") { //SOLO SI ES LOCALHOST*/
    ...
    //$email->SMTPDebug = 2;
    $email->Port = 26; //25
    $email->Host = 'mail.genotipo.com';
    $email->Username = 'sistema@genotipo.com';
    $email->Password = 'G3n0W3b!';
  }
  /* - - - - - CONFIGURACION LOCAL PARA ENVIAR CORREOS - - - - - */

  $email->From = 'contacto@coantic.com';
  $email->FromName = 'Coantic Company';
  $email->Subject = "Prospecto de Coantic Company";
  $email->Body = $mensaje;
  $email->isHTML(true);
  $email->CharSet = 'UTF-8';
  ...
  /********************************
                CON COPIA OCULTA
   *********************************/
  $limpiaCCO_correos = str_replace(' ', '', $copiaCCO);
  $correosCCO = explode(",", $limpiaCCO_correos);

  foreach ($correosCCO as $correoCCO) {
    $email->addBCC($correoCCO);
  }
